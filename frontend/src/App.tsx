import './app.scss'
import { useState } from 'react'
import ApartmentList from './components/ApartmentList'
import { ParsedApartmentInfo, SearchOptions } from '../../types'
import { getApartmentInfos } from './utils/apartmentParsers'
import Tabs from './components/Tabs'

const App = () => {
  const [searchOptions, setSearchOptions] = useState<SearchOptions>({
    generalFilterSettings: [],
    travelTimeFilterSettings: [],
  })

  const apartmentInfos = getApartmentInfos(searchOptions?.loanSettings)

  const filterApartments = (apartment: ParsedApartmentInfo): boolean => {
    let keep = true
    const filterSettings = [
      ...searchOptions.generalFilterSettings.map(({ name, min, max }) => ({
        value: apartment[name],
        min,
        max,
      })),
      ...searchOptions.travelTimeFilterSettings.map(({ name, min, max }) => ({
        value: apartment.travelTimes.find(({ destination }) => destination === name)?.duration,
        min,
        max,
      })),
    ]
    filterSettings.forEach(({ value, min, max }) => {
      if (value) {
        if (max && value > max) {
          keep = false
        }
        if (min && value < min) {
          keep = false
        }
      }
    })
    return keep
  }

  const filteredApartmentInfos = apartmentInfos
    .filter(filterApartments)
    .sort((a, b) => a.totalFees - b.totalFees)

  return (
    <div className="pageBackground">
      <div className="main">
        <Tabs
          setSearchOptions={setSearchOptions}
          apartmentInfos={apartmentInfos}
          searchOptions={searchOptions}
        />
        <h2>Asuntoja: {filteredApartmentInfos.length}</h2>
        <ApartmentList apartmentInfos={filteredApartmentInfos.slice(0, 30)} />
      </div>
    </div>
  )
}

export default App
